...
function match_line(text) {
    let obj = {
        line: text,
        sigil: 'none', // or leading, or extend
        leading: '',
        ws_pre: '',
        first: '',
        ws_post: '',
...
export default {
    ...
    invoke(event) {
        ...
        let text = this.reflow(
            ...
            {
                line_len: atom.config.get('editor.preferredLineLength', {scope: editor.getRootScopeDescriptor()}),
                tab_len: atom.config.get('editor.tabLength', {scope: editor.getRootScopeDescriptor()})
            }
        );
    ...
    reflow(text, state) {
        ...
        if (! state.tab_len) state.tab_len = 8;

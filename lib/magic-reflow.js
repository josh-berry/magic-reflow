...
const LEADING_PATT = /^(\s*(-|\+|\*|<!--|\/\*|\(\*|[0-9]+\.|[a-z]+\.|[A-Z]+\.|\([0-9]+\)|\([a-z]+\)|\([A-Z]+\))\s+)(.*)$/;
...
export default {
    ...
    reflow_lines: function*(lines, lineLength, tabLength) {
        ...
        // Now break into paragraphs and reflow each paragraph separately.  Look
        // for lists and other block elements.
        let block_state = 'blank';
        let para = [];
        ...
        for (let l of lines) {
            ...
            if (l.search(LEADING_PATT) !== -1) {
                if (block_state == 'blank') block_state = 'block';
                if (block_state == 'block') {
                    // Force-start a new paragraph here; this is probably a new
                    // list item.
                    yield* this.reflow_para(para, lineLength, tabLength);
                    para = [l];
                    continue;
            ...
            if (block_state == 'blank') block_state = 'para';
            para.push(l);
        }
    ...
    reflow_para: function*(lines, lineLength, tabLength) {
        ...
        let m;
        if (m = lines[0].match(LEADING_PATT)) {
        ...
        } else if (m = lines[0].match(LEADING_WS_PATT)) {
        ...
        console.log(`first: "${firstDecorator}"`);
        console.log(`rest: "${restDecorator}"`);
    ...
    common_prefix(lines) {
        ...
        // Things in LEADING_PATT are always leading sigils, not prefixes, since
        // we treat them as list items or other block elements.
        if (lines[0].search(LEADING_PATT) !== -1) return '';

        if (lines.length == 1) {
            // We only have one line to work with, so need to guess if we have a
            // common prefix.  Things in PREFIX_PATT (but not LEADING_PATT) are
            // usually prefixes.
            if (m = lines[0].match(PREFIX_PATT)) return m[1];
            if (m = lines[0].match(LEADING_WS_PATT)) return m[1];
            return '';
        }
